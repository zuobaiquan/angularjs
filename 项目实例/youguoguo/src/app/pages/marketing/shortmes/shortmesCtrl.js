(function() {
    ...
    angular.module('BlurAdmin.pages.marketing')
        .controller('shortmesCtrl', shortmesCtrl);

    function shortmesCtrl($scope,toastr,apiRequest,baUtil,modalsService) {
        $scope.smartTable = {
            pageSize: 10,
            searchContent: '',
            start:"",
        ...
        $scope.getList = function(tableState) {
            ...
            apiRequest.post('/admin/notePush/notePush-list', params, function(res) {
        ...
        $scope.showNotetarget=function(target){
            if(target.length>1){
                return "用户端、商家端";
            }
            if(target=="1"){
        ...
        $scope.del = function(id) {
            $scope.modal = {
                url: "/admin/notePush/update-notePushStatus",
                title: "确认信息",
                success: "删除成功",
                message: "您确定删除您选中的项吗？",
                params: {
