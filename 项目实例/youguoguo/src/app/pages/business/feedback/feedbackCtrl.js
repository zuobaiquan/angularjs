(function() {
    ...
    function feedbackCtrl($scope,apiRequest,toastr,baUtil,modalsService) {
        $scope.smartTable = {
            pageSize: 5,
            searchContent: '',
            start:"",
        ...
        $scope.getList = function(tableState) {
            ...
            apiRequest.post('/admin/feedback/feedbackList', params, function(res) {
        ...
        $scope.feedbackDetail=function(id){
            $scope.modal = {
                params: {
                    id:id,
                    reason:''
            ...
            modalsService.normal('app/pages/business/feedback/addfeedback.html','feedbackeditCtrl',$scope.modal,function(){
                $scope.getList($scope.tableState);
            });
        }
        ...
        $scope.exportExcel=function(){
            ...
            if($scope.smartTable.start!=""){
                param.startTime=$scope.smartTable.start+" "+"00:00:00";
            }
            if($scope.smartTable.end!=""){
                param.endTime=$scope.smartTable.end+" "+"00:00:00";
            }
            if($scope.smartTable.searchContent!=''){
                param.key=$scope.smartTable.searchContent;
            }
            apiRequest.post('/admin/feedback/exportFeedback', param, function(res) {
                $scope.exportlink=res.data;
                location.href=$scope.exportlink;
            });
        }
    }
