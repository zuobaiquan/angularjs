(function() {
    ...
    function addareaCtrl($scope,toastr, apiRequest,modal,$uibModalInstance) {
        $scope.disable = false;
        $scope.modal = modal;
        ...
        $scope.selValue=function(index){
            if(index!='hot'){
                ...
                for(var j=0;j<index.length;j++){
                    $scope.list = $scope.cityAll.filter(function(item) {
                ...
                $scope.isHot=true;
                $scope.allcityList = $scope.cityAll.filter(function(item) {
                    return item.name==index;
                });
            }
        }

        $scope.selItemname=function(name){
            $scope.modal.params.cityName=name;
            angular.element("body").find("#showCityname").css({"display":"none"});
        }
