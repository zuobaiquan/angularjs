(function() {
    ...
    angular.module('BlurAdmin.pages.pumpmoney')
        .controller('pumpCtrl', pumpCtrl);

    /** @ngInject */
    function pumpCtrl($scope,apiRequest,modalsService) {
        $scope.companyType =[
            { id: 0, des: "全部" },
            { id: 1, des: "店家" },
            { id: 2, des: "达人" }
        ...
        $scope.getList = function(tableState) {
            ...
            apiRequest.post('/admin/pumpmoney/pumpmoneyList', params, function(res) {
        ...
        $scope.editPump = function(item){
            $scope.modal = {
                url: "/admin/pumpmoney/set-pumpmoney",
                title: "抽成设置",
                success: "设置成功",
                data:item,
                params: {
            ...
            modalsService.normal('app/pages/pumpmoney/pump/editpump.html','ModalCtrl',$scope.modal,function(){
                $scope.getList($scope.tableState);
            });
        }
    }
